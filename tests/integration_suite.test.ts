...
async function rodarTestes() {
  ...
  try {
    ...
    assert(garoonRes.ok && String(garoonRes.garoonRecordId).startsWith('GRN-'), 'Garoon Sync Cloud REST envia payload com sucesso');
  } catch (err: any) {
  ...
  try {
    const garoonOnPrem = await executarSincronizacaoGaroon({
      subdomain: 'fujiarte-onprem',
      usuario: 'admin_soap',
      apiToken: 'soap_pass_2026',
      ambiente: 'on_premise',
      candidato: {
        nome_completo: 'Hanako Sato',
        cpf: '987.654.321-11',
        passaporte: 'TK112233',
      },
    });
    assert(garoonOnPrem.ok && String(garoonOnPrem.payloadEnviado).includes('<SOAP-ENV:Envelope'), 'Garoon Sync On-Premise gera envelope SOAP XML válido');
  } catch (err: any) {
    assert(false, `Garoon OnPremise erro: ${err.message}`);
  }

  // ── TESTE 6: Sitemap XML Generator ──
  try {
